...
from torch_mimicry.modules.slimmable_ops import FLAGS, \
    SwitchableBatchNorm2d, SwitchableConditionalBatchNorm2d, SliceableConditionalBatchNorm2d, \
    SlimmableConv2d, SNSlimmableConv2d
...
class SlimmableGBlock(nn.Module):
    r"""
    Residual block for generator.
    ...
    def __init__(self,
        ...
        # this results in somehow spectral norm working worse consistently.
        if self.spectral_norm:
            self.c1 = SNSlimmableConv2d([int(self.in_channels * mult) for mult in FLAGS.width_mult_list],
                                        [int(self.hidden_channels * mult) for mult in FLAGS.width_mult_list],
                                        3,
                                        1,
                                        padding=1)
            self.c2 = SNSlimmableConv2d([int(self.hidden_channels * mult) for mult in FLAGS.width_mult_list],
                                        [int(self.out_channels * mult) for mult in FLAGS.width_mult_list],
                                        3,
                                        1,
                                        padding=1)
        else:
            self.c1 = SlimmableConv2d([int(self.in_channels * mult) for mult in FLAGS.width_mult_list],
                                      [int(self.hidden_channels * mult) for mult in FLAGS.width_mult_list],
                                      3,
                                      1,
                                      padding=1)
            self.c2 = SlimmableConv2d([int(self.hidden_channels * mult) for mult in FLAGS.width_mult_list],
                                      [int(self.out_channels * mult) for mult in FLAGS.width_mult_list],
                                      3,
                                      1,
                                      padding=1)
        ...
        if self.num_classes == 0:
            self.b1 = SwitchableBatchNorm2d([int(self.in_channels * mult) for mult in FLAGS.width_mult_list])
            self.b2 = SwitchableBatchNorm2d([int(self.hidden_channels * mult) for mult in FLAGS.width_mult_list])
        else:
            self.b1 = SliceableConditionalBatchNorm2d([int(self.in_channels * mult) for mult in FLAGS.width_mult_list],
                                                          self.num_classes)
            self.b2 = SliceableConditionalBatchNorm2d([int(self.hidden_channels * mult) for mult in FLAGS.width_mult_list],
                                                          self.num_classes)
        ...
        if self.learnable_sc:
            if self.spectral_norm:
                self.c_sc = SNSlimmableConv2d([int(self.in_channels * mult) for mult in FLAGS.width_mult_list],
                                              [int(self.out_channels * mult) for mult in FLAGS.width_mult_list],
                                              1,
                                              1,
                                              padding=0)
            else:
                self.c_sc = SlimmableConv2d([int(self.in_channels * mult) for mult in FLAGS.width_mult_list],
                                            [int(self.out_channels * mult) for mult in FLAGS.width_mult_list],
                                            1,
                                            1,
                                            padding=0)
