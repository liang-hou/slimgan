...
from torch_mimicry.metrics import compute_fid, compute_is, compute_kid, compute_mic
...
def evaluate(metric,
    ...
    if metric == 'kid':
        ...
        if 'num_samples' not in kwargs:
            raise ValueError(
                "num_samples must be provided for mIC computation.")

        output_file = os.path.join(
            log_dir,
            'mic_{}k.json'.format(kwargs['num_samples'] // 1000))

    else:
        choices = ['fid', 'kid', 'inception_score', 'mic']
    ...
    for step in range(start, end + 1, interval):
        ...
        for seed in range(start_seed, start_seed + num_runs):
            ...
            if metric == "fid":
                ...
                score = compute_mic.mic_score(netG=netG,
                                              seed=seed,
                                              device=device,
                                              log_dir=log_dir,
                                              **kwargs)
