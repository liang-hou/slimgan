...
class SlimmableSNDCGANGenerator64(slimmable_sndcgan_base.SlimmableSNDCGANBaseGenerator):
    r"""
    DCGAN backbone generator for SNDCGAN.
    ...
    def __init__(self, nz=128, ngf=1024, bottom_width=4,
        ...
        self.c4 = SlimmableConvTranspose2d(
            [int(self.ngf * width_mult) >> 3 for width_mult in FLAGS.width_mult_list],
            [int(self.ngf * width_mult) >> 4 for width_mult in FLAGS.width_mult_list],
            4, 2, 1
        )
        self.b4 = SwitchableBatchNorm2d([int(self.ngf * width_mult) >> 4 for width_mult in FLAGS.width_mult_list])

        self.c5 = SlimmableConv2d(
            [int(self.ngf * width_mult) >> 4 for width_mult in FLAGS.width_mult_list],
...
class SlimmableSNDCGANDiscriminator64(slimmable_sndcgan_base.SlimmableSNDCGANBaseDiscriminator):
    r"""
    ...
    def __init__(self, ndf=1024, n_share=0, **kwargs):
        ...
        assert -1 <= self.n_share <= 9

        # Build layers

        self.c1s = nn.ModuleList([
            SNConv2d(3, self.ndf >> 4, 3, 1, 1)
            for width_mult in FLAGS.width_mult_list
        ])
        self.c2s = nn.ModuleList([
            SNConv2d(self.ndf >> 4, self.ndf >> 4, 4, 2, 1)
            for width_mult in FLAGS.width_mult_list
        ])

        self.c3s = nn.ModuleList([
            SNConv2d(self.ndf >> 4, self.ndf >> 3, 3, 1, 1)
            for width_mult in FLAGS.width_mult_list
        ])
        ...
        self.c8s = nn.ModuleList([
            SNConv2d(self.ndf >> 1, self.ndf >> 1, 4, 2, 1)
            for width_mult in FLAGS.width_mult_list
        ])

        self.c9s = nn.ModuleList([
            SNConv2d(self.ndf >> 1, self.ndf, 3, 1, 1)
            for width_mult in FLAGS.width_mult_list
        ])

        self.l10s = nn.ModuleList([
            SNLinear(self.ndf * 4 * 4, 1)
            for width_mult in FLAGS.width_mult_list
        ])
        ...
        for i in range(len(FLAGS.width_mult_list)):
            ...
            nn.init.xavier_uniform_(self.c8s[i].weight.data, 1.0)
            nn.init.xavier_uniform_(self.c9s[i].weight.data, 1.0)
            nn.init.xavier_uniform_(self.l10s[i].weight.data, 1.0)
    ...
    def forward(self, x):
        ...
        h = self.activation(self.c8s[-1 if self.n_share > 7 else idx](h))
        h = self.activation(self.c9s[-1 if self.n_share > 8 else idx](h))

        h = h.view(h.shape[0], -1)

        output = self.l10s[-1 if self.n_share == -1 else idx](h)
