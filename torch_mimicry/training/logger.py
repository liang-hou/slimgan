...
class Logger:
    ...
    def show_res_images(self, netG, global_step, num_images=64):
        ...
        with torch.no_grad():
            ...
            for width_mult in FLAGS.width_mult_list:
                ...
                # Map name to results
                images_dict = {
                    'fixed_fake'.format(width_mult): fixed_fake_images,
                }
                ...
                for name, images in images_dict.items():
                    ...
                    imgs.append(images[:16])

                    vutils.save_image(images_viz,
                                      '{}/{}_samples_step_{}_width_{:.2f}.png'.format(
                                          img_dir, name, global_step, width_mult),
                                      normalize=True)

                    if 'img' not in self.writers:
                        self.writers['img'] = self._build_writer('img')

                    self.writers['img'].add_image('{}_vis_width_{:.2f}'.format(name, width_mult),
                                                  images_viz,
                                                  global_step=global_step)

            imgs = torch.cat(imgs)
            imgs_viz = vutils.make_grid(imgs, padding=2, normalize=True)
            vutils.save_image(imgs_viz, '{}/consistency_step_{}.png'.format(img_dir, global_step),
                              normalize=True)
